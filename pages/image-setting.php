...
<div class="wrap">
  <div class="form-wrap">
	...
	if (isset($_POST['ssg_submit'])) 
	{
		...
		$ssg_title = stripslashes(sanitize_text_field($_POST['ssg_title']));
		$ssg_width = intval($_POST['ssg_width']);
		$ssg_height = intval($_POST['ssg_height']);
		$ssg_pause = intval($_POST['ssg_pause']);
		$ssg_fadeduration = intval($_POST['ssg_fadeduration']);
		$ssg_descreveal = stripslashes(sanitize_text_field($_POST['ssg_descreveal']));
		$ssg_descreveal1 = stripslashes(sanitize_text_field($_POST['ssg_descreveal1']));
		$ssg_cycles = intval($_POST['ssg_cycles']);
		$ssg_random = stripslashes(sanitize_text_field($_POST['ssg_random']));
		$ssg_type = stripslashes(sanitize_text_field($_POST['ssg_type']));
			
		if($ssg_descreveal != "always" && $ssg_descreveal != "peek-a-boo")
		{
			$ssg_descreveal = "always";
		}
		
		if($ssg_descreveal1 != "always" && $ssg_descreveal1 != "peek-a-boo")
		{
			$ssg_descreveal1 = "peek-a-boo";
		}
		
		if($ssg_random != "YES" && $ssg_random != "NO")
		{
			$ssg_random = "YES";
		}
    ...
    <form name="ssg_form" method="post" action="">
      ...
      <p><?php _e('Only Number / Pause time of the slideshow in milliseconds. (Example: 2500)', 'superb-slideshow-gallery'); ?></p>
      ...
      <p><?php _e('Only Number / Duration of transition (affects only IE users) (Example: 500)', 'superb-slideshow-gallery'); ?></p>
      ...
      <input name="ssg_cycles" id="ssg_cycles" type="text" value="<?php echo $ssg_cycles; ?>" maxlength="1" />
